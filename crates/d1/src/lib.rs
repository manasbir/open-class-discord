...
pub async fn get_open_classes(db: D1Database, params: Params) -> Result<Vec<SQLRes>> {
    ...
    add_param(&mut query_string, &mut param_vec, params.floor_number, "AND f.floor_number = ?");
    add_param(&mut query_string, &mut param_vec, params.room_number, "AND r.room_number = ?");
    ...
    let params = param_vec.into_iter().map(|param| param.into()).collect::<Vec<_>>();

    Ok(db.prepare(query).bind(&params)?.all().await?.results::<SQLRes>()?)
}

fn add_param<'a, T: ToString>(query_string: &mut Vec<&'a str>, params: &mut Vec<String>, value: Option<T>, string: &'a str) {
